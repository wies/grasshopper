...
`column'
     matches the start column number.

`end-column'
     matches the end column number.
    ...
    (let ((rx-constituents
           (append
            `((line . ,(rx (group-n 2 (one-or-more digit))))
              ...
              (end-column . ,(rx (group-n 4 (one-or-more digit))))
...
(cl-defstruct (flycheck-error
               ...
               (:constructor flycheck-error-new-at (line column end-column
...
`end-column'
     The end column number the error refers to, as number.

`level'
     The error level, as either `warning' or `error'."
  buffer checker filename line column end-column message level)
...
(defun flycheck-error-end-column-region (err)
          ...
          (let ((line (flycheck-error-line err)))
            ...
            (cond
             ...
             (:else
              ...
              ;; of the line.
              (let ((start (min (+ (line-beginning-position) column)
                                (+ (line-end-position) 1)))
                    (end (min (+ (line-beginning-position) end-column)
                              (+ (line-end-position) 1))))
                (cons start end))))))))))
...
Otherwise signal an error."
  (pcase mode
    (`columns (or (flycheck-error-end-column-region err)
                  (flycheck-error-column-region err)
  ...
  (let* ((regexp (car pattern))
    ...
    (when match
      (flycheck-error-new
       ...
       :end-column (flycheck-match-int 4 match)
       :message (flycheck-match-string-non-empty 5 match t)
       :level level))))
    ...
    (->> (cddr node)
      ...
      (--map
       (let* ((attrs (cadr it))
              ...
              (end-column (flycheck-string-to-number-safe
                       (cdr (assq 'end-column attrs))))
         ...
         (flycheck-error-new
          ...
          :end-column (when (and end-column (> end-column 0)) column)
          :message message
